...
polar_graph.createCanvasElements = function(){
  ...
  polar_graph.line = polar_graph.origin.append('line').attrs({ x1: 0, y1: 0, 'marker-end': 'url(#arrow)' }).styles({ 'stroke': 'white', 'stroke-width': 3 });

  polar_graph.winding_freq_ctlr = {};
  polar_graph.winding_freq_ctlr.div = d3.select('#container').append('div').styles({ 'position': 'absolute', 'top': polar_graph.height+cartesian_graph.height+30, 'left': 0 });
  polar_graph.winding_freq_ctlr.slider = polar_graph.winding_freq_ctlr.div.append('input').attrs({ type: 'range', min: 0.2, max: 2, step: 0.01, value: waves[0].freq }).styles({ width: '10em' });
  polar_graph.winding_freq_ctlr.text = polar_graph.winding_freq_ctlr.div.append('text').attrs({ class: 'ml-2' }).styles({ 'font-size': '1.5em', 'color': '#bbb', 'vertical-align': 'text-bottom' }).html('Winding Frequency = ' + winding_freq + ' Hz');
  polar_graph.winding_freq_ctlr.slider.on('input', function(){
    winding_freq = this.value;
    polar_graph.winding_freq_ctlr.text.html('Winding Frequency = ' + winding_freq + ' Hz');
    update();
  })
...
polar_graph.animate = function(){
  ...
  ctx.lineWidth = 3; ctx.strokeStyle = 'yellow';
    ...
    if(winding_animation.index != 0){
      var com_x = math.mean( this.x_array.slice(0, winding_animation.index) );
      var com_y = math.mean( this.y_array.slice(0, winding_animation.index) );
      this.circle.attrs({ cx: com_x, cy: com_y });
    }
}
